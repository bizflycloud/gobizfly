...
func (c *kubernetesEngineService) GetEverywhere(
	ctx context.Context,
	id string,
) (*EverywhereNode, error) {
	var everywhereNode *EverywhereNode
	var kubernetesErrorResponse *KubernetesErrorResponse
	req, err := c.client.NewRequest(ctx, http.MethodGet, kubernetesServiceName, c.EverywherePath(id), nil)
	if err != nil {
		return nil, &KubernetesEngineError{status: http.StatusInternalServerError, msg: err.Error()}
	}
	resp, err := c.client.Do(ctx, req)
	if err != nil {
		return nil, &KubernetesEngineError{status: http.StatusInternalServerError, msg: err.Error()}
	...
	if resp.StatusCode != http.StatusOK {
		if err := json.NewDecoder(resp.Body).Decode(&kubernetesErrorResponse); err != nil {
			return nil, &KubernetesEngineError{status: http.StatusInternalServerError, msg: err.Error()}
		}
		return nil, &KubernetesEngineError{status: http.StatusNotFound, msg: kubernetesErrorResponse.Message}
	}
